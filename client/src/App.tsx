  ...
  function onGenerate() {
    ...
    let candidates = rows.filter((r) => {
      ...
      if (product === "EDU") {
        ...
        if (secTag !== "unknown" && secTag !== eduSecurity && secTag !== "both") return false;
        const locTag = inferEduLocale(r);
        if (locTag !== "unknown" && locTag !== eduLocale && locTag !== "both") return false;
      }
      // Loan caps
    ...
    const enriched = candidates.map((r) => {
      ...
      if (isNTBMatch && haveAmt) {
        try {
          console.log("NTB processing:", { bank: r.bank, product, monthlyNetSalary, salaryLevel, typePref, tenureYears, rowType: r.type, rowTenureLabel: (r as any).tenureLabel });
          ...
          if (product === "PL") {
            ...
            if (r.type === "Floating" && (typePref === "floating" || typePref === "any")) {
              if ((r as any).tenureLabel === "Anytime variable") {
                isMatchingRow = true;
              ...
              if (tenureYears === 1 && (r as any).tenureLabel === "01 year fixed") {
                isMatchingRow = true;
                rateDisplay = "Fixed 1 year";
              } else if (tenureYears >= 2 && tenureYears <= 5 && (r as any).tenureLabel === "Up to 5 years") {
                isMatchingRow = true;
                rateDisplay = "Fixed up to 5 years";
              } else if (tenureYears > 5 && (r as any).tenureLabel === "Above 5 years") {
                isMatchingRow = true;
            ...
            if (r.type === "Fixed" && (typePref === "fixed" || typePref === "any")) {
              if (tenureYears <= 1 && (r as any).tenureLabel === "01 year fixed") {
                isMatchingRow = true;
                rateDisplay = "Fixed 1 year";
              } else if (tenureYears > 1 && (r as any).tenureLabel === "05 year fixed") {
                isMatchingRow = true;
              ...
              if (!isNaN(parsedRate)) {
                ...
                scenarioSel = {
                  ...
                  scenario: {
                    rate: baseRate,
                    key: "ntb_best_match",
                    minSalary: qualifiesForSalaryRate ? "remittance" : "none",
                    label: rateDisplay,
                  },
                  eligible: true,
                };
    ...
    const picks: typeof validEnriched = [];
    const seen = new Set<string>();
        ...
        return savedComparisons.length > 0 && (
          <div className="rounded-xl p-4 border border-white/10" style={{ backgroundColor: BRAND.card }}>
            ...
            <div className="grid gap-2 max-h-40 overflow-y-auto">
              {savedComparisons.slice(0, 5).map((bookmark: any) => (
                <div 
                  key={bookmark.id} 
                  ...
                  <div className="grid md:grid-cols-2 gap-3">
                    {(() => {
                      const improvements = [
                        ...
                        {
                          condition: takeCreditCard === "no" && product !== "EDU",
                          icon: "💳",
